...
export default function Nav() {
	...
	const links = [
		...
		[t.nav.experience, "EXPERIENCES"],
		[t.nav.projects, "PROJECT"],
		...
		<header className="fixed inset-x-0 top-0 z-50 border-b border-white/10 bg-[#0b0724]/85 text-white backdrop-blur-xl">
			<div className="mx-auto flex h-[74px] max-w-7xl items-center justify-between px-5 sm:px-8 lg:px-12">
				...
				<div className="flex items-center gap-2">
					...
					<a href="https://github.com/Abdelmounaim08" target="_blank" rel="noreferrer" className="hidden rounded-full border border-white/15 px-4 py-2 text-xs font-bold text-white/80 transition hover:border-[#f5d35f]/60 hover:text-[#f5d35f] sm:block">GitHub ↗</a>
			...
			{open && (
				<div className="border-t border-white/10 bg-[#0b0724] md:hidden">
					<div className="mx-auto max-w-7xl space-y-2 px-5 py-4 sm:px-8">
						...
						<div className="nav-link-mobile"><Link onClick={() => setOpen(false)} href="#service">{t.nav.services}</Link></div>
						<div className="nav-link-mobile"><Link onClick={() => setOpen(false)} href="#EXPERIENCES">{t.nav.experience}</Link></div>
						<div className="nav-link-mobile"><Link onClick={() => setOpen(false)} href="#PROJECT">{t.nav.projects}</Link></div>
